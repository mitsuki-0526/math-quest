...
function genSector(rng: Rng, d: Difficulty): Problem {
  const r = rng.pick([3, 4, 6, 8, 9, 10, 12]);
  const angle = rng.pick([30, 45, 60, 90, 120, 135, 150, 180, 270]);
...
const CONSTRUCTS = [
  ...
  { q: '線分 AB を 3等分する 作図で 使うのは?', a: '平行線を 使った 等しい 間かくの とり方', w: ['垂直二等分線を 2回 引く', '角の 二等分線', '接線'] },
